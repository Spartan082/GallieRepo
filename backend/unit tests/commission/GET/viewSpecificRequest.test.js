...
describe("Database Info Tests", () => {
  ...
  beforeEach(async () => {
    ...
    let createTestRequestTableSQL =
      "CREATE TABLE `testViewSpecificRequest` ( `id` INT(9) ZEROFILL , `customerEmail` VARCHAR(30) , `artistUsername` VARCHAR(30) , `prodName` VARCHAR(30) , `initialPrice` DECIMAL(8,2) , `prodDesc` VARCHAR(250) , `postDate` VARCHAR(50) , PRIMARY KEY (`id`)) ENGINE = InnoDB;";
  ...
  it("Test Querying Specific Request from DB", async () => {
    try {
      let insertQueries = [];
    
      //populate testViewSpecificRequest table with test data
      let insertSQL = `INSERT INTO testViewSpecificRequest (id,  customerEmail, artistUsername, prodName, initialPrice, prodDesc, postDate) VALUES (${requestID}, '${email}', '${username}', '${prodName}', ${price}, '${prodDesc}', '${date}');`;
      insertQueries.push(connection.query(insertSQL));

      await Promise.all(insertQueries);
    
      //query the test tables on the db
      const [rows, fields] = await connection.query("SELECT * FROM testViewSpecificRequest WHERE id = '" + requestID + "'");
      ...
      let dropTestRequestTableSQL = "DROP TABLE IF EXISTS `testViewSpecificRequest`";
