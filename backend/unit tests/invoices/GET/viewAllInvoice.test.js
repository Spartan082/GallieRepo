...
describe("Database Info Tests", () => {
  ...
  let status2 = 'Test Status 2';
  let date2 = '2020-05-14T11:01:58.135Z';

  beforeEach(async () => {
    //create mock mysql tables
    let createTestAllInvoicesTableSQL =
      "CREATE TABLE `testViewAllInvoices` ( `id` INT(9) ZEROFILL , `artistEmail` VARCHAR(30) , `customerEmail` VARCHAR(30),  `prodCost` DECIMAL(8,2),  `prodDesc` VARCHAR(250) , `paymentType` VARCHAR(30) , `paymentStatus` VARCHAR(30) ,`postDate` VARCHAR(50) , PRIMARY KEY (`id`)) ENGINE = InnoDB;";
    ...
    await connection.query(createTestAllInvoicesTableSQL);
  });

  it("Test Querying All Invoices from DB", async () => {
    try {
      let insertQueries = [];
    
      //populate testViewAllInvoices table with report 1
      let insertSQL1 = `INSERT INTO testViewAllInvoices (id, artistEmail, customerEmail, prodCost, prodDesc, paymentType, paymentStatus, postDate) VALUES (${id}, '${artistEmail}', '${customerEmail}', '${price}', '${prodDesc}', '${type}', '${status}', '${date}');`;
      insertQueries.push(connection.query(insertSQL1));

      //populate testViewAllInvoices table with report 2
      let insertSQL2 = `INSERT INTO testViewAllInvoices (id, artistEmail, customerEmail, prodCost, prodDesc, paymentType, paymentStatus, postDate) VALUES (${id2}, '${artistEmail2}', '${customerEmail2}', '${price2}', '${prodDesc2}', '${type2}', '${status2}', '${date2}');`;
      insertQueries.push(connection.query(insertSQL2));

      await Promise.all(insertQueries);
    
      //query the test tables on the db
      const [rows, fields] = await connection.query("SELECT * FROM testViewAllInvoices ORDER BY postDate DESC");
      ...
      expect(rows[0].paymentStatus).toBe(status2);
      ...
      let dropTestAllInvoicesTableSQL = "DROP TABLE IF EXISTS `testViewAllInvoices`";
      await connection.query(dropTestAllInvoicesTableSQL);
