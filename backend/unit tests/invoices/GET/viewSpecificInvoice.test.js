...
describe("Database Info Tests", () => {
  ...
  beforeEach(async () => {
    ...
    let createTestInvoicesTableSQL =
      "CREATE TABLE `testViewSpecificInvoice` ( `id` INT(9) ZEROFILL , `artistEmail` VARCHAR(30) , `customerEmail` VARCHAR(30),  `prodCost` DECIMAL(8,2),  `prodDesc` VARCHAR(250) , `paymentType` VARCHAR(30) , `paymentStatus` VARCHAR(30) ,`postDate` VARCHAR(50) , PRIMARY KEY (`id`)) ENGINE = InnoDB;";
  ...
  it("Test Querying Specific Invoice from DB", async () => {
    try {
      let insertQueries = [];
    
      //populate testViewSpecificInvoice table with test data
      let insertSQL = `INSERT INTO testViewSpecificInvoice (id, artistEmail, customerEmail, prodCost, prodDesc, paymentType, paymentStatus, postDate) VALUES (${id}, '${artistEmail}', '${customerEmail}', '${price}}', '${prodDesc}', '${type}', '${status}', '${date}');`;
      insertQueries.push(connection.query(insertSQL));

      await Promise.all(insertQueries);
    
      //query the test tables on the db
      const [rows, fields] = await connection.query("SELECT * FROM testViewSpecificInvoice WHERE id = '" + id + "'");
      ...
      let dropTestInvoicesTableSQL = "DROP TABLE IF EXISTS `testViewSpecificInvoice`";
