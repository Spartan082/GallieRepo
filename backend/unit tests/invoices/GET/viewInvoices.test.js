...
describe("Database Info Tests", () => {
  ...
  beforeEach(async () => {
    ...
    let createTestInvoicesTableSQL =
      "CREATE TABLE `testViewInvoices` ( `id` INT(9) ZEROFILL , `artistEmail` VARCHAR(30) , `customerEmail` VARCHAR(30),  `prodCost` DECIMAL(8,2),  `prodDesc` VARCHAR(250) , `paymentType` VARCHAR(30) , `paymentStatus` VARCHAR(30) ,`postDate` VARCHAR(50) , PRIMARY KEY (`id`)) ENGINE = InnoDB;";
  ...
  it("Test Querying Invoice from DB", async () => {
    try {
      let insertQueries = [];
    
      //populate testViewInvoices table with report 1
      let insertSQL1 = `INSERT INTO testViewInvoices (id, artistEmail, customerEmail, prodCost, prodDesc, paymentType, paymentStatus, postDate) VALUES (${id}, '${artistEmail}', '${customerEmail}', '${price}}', '${prodDesc}', '${type}', '${status}', '${date}');`;
      insertQueries.push(connection.query(insertSQL1));

      //populate testViewInvoices table with report 1
      let insertSQL2 = `INSERT INTO testViewInvoices (id, artistEmail, customerEmail, prodCost, prodDesc, paymentType, paymentStatus, postDate) VALUES (${id2}, '${artistEmail}', '${customerEmail2}', '${price2}}', '${prodDesc2}', '${type2}', '${status2}', '${date2}');`;
      insertQueries.push(connection.query(insertSQL2));

      await Promise.all(insertQueries);
    
      //query the test tables on the db
      const [rows, fields] = await connection.query("SELECT * FROM testViewInvoices WHERE artistEmail = '" + artistEmail + "' ORDER BY postDate DESC");
      ...
      let dropTestInvoicesTableSQL = "DROP TABLE IF EXISTS `testViewInvoices`";
