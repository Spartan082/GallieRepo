...
describe("Database Info Tests", () => {
  ...
  beforeEach(async () => {
    ...
    let createTestTemplateTableSQL =
      "CREATE TABLE `testTemplateFlatColor` ( `id` INT(9) ZEROFILL , `artType` VARCHAR(30) NOT NULL , `artPrice` DECIMAL(5,2) NOT NULL , `artDesc` VARCHAR(30000) NOT NULL , `artExURL` VARCHAR(250) , `postDate` VARCHAR(50) NOT NULL , PRIMARY KEY (`id`)) ENGINE = InnoDB;";
  ...
  it("Test Querying Template for FlatColor from DB", async () => {
    try {
      ...
      let insertSQL1 = `INSERT INTO testTemplateFlatColor (id,  artType, artPrice, artDesc, artExURL, postDate) VALUES (${id}, '${artType}', '${artPrice}', '${artDesc}', '${artURL}', '${date}');`;
      insertQueries.push(connection.query(insertSQL1));

      //populate testProfile table with art 2
      let insertSQL2 = `INSERT INTO testTemplateFlatColor (id,  artType, artPrice, artDesc, artExURL, postDate) VALUES (${id2}, '${artType}', '${artPrice2}', '${artDesc2}', '${artURL2}', '${date2}');`;
      insertQueries.push(connection.query(insertSQL2));
      
      await Promise.all(insertQueries);
    
      //query the test tables on the db
      const [rows, fields] = await connection.query("SELECT * FROM testTemplateFlatColor WHERE artType = 'Flat Color' ORDER BY postDate DESC LIMIT 1;");
      ...
      let dropTestTemplateTableSQL = "DROP TABLE IF EXISTS `testTemplateFlatColor`";
